  ...
  class Picture < Liquid::Tag
    ...
    def render(context)
        ...
        instance.each { |key, source|
          ppi.each { |p|
            ...
            ppi_sources[ppi_key] = {
              ...
              'media' => p != 1 ? "#{source[:width]}w #{p}x" : "#{source[:width]}w",
              :src => source[:src]
            }
            ...
            if p != 1
              ...
              source_keys[source_keys.index(key)] = ppi_key
            end
      ...
      if settings['markup'] == 'picture'
        ...
        source_tags = ''
        source_keys.reverse.each_with_index { |source, index|
          media = "#{instance[source]['media']}"
          source_tags += "#{instance[source][:generated_src]} #{media}"
